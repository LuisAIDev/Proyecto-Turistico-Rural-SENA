...
function Navbar() {
  ...
  // LISTA COMPLETA CON TODOS TUS MÓDULOS EXISTENTES
  const menuItems = [
    {
      name: 'Dashboard',
      path: '/dashboard',
    ...
    },
    {
      ...
      path: '/servicios', // Apunta a la ruta exacta de tu catálogo
  ...
  return (
    <aside className="w-64 bg-[#14532D] text-white min-h-screen p-4 flex flex-col shadow-xl fixed left-0 top-0 z-40">
      {/* Logo y Título */}
      <div className="mb-10 px-4 pt-4">
        ...
        <p className="text-[10px] text-green-300/50 font-bold mt-2 uppercase tracking-[0.2em]">
          Administrador
        </p>
      </div>
          ...
          return (
            <Link
              ...
              {item.name === 'Facturación' && (
                <TrendingUp
                  size={14}
                  className="ml-auto text-green-400 opacity-50"
                />
              )}
            </Link>
          );
